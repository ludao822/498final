...
class Pilot (Ckpt.Ckpt):  # subclass of the class Ckpt in the file Ckpt
    ...
    def ai(sf,fDat,fCmd,desired_altitude_change = 1500, desired_gamma = 12, desired_velocity=200):
        if not fDat.running:
    ...
    def DO(self,fDat,fCmd):
        ...
        if self.state is "Initialize":
            ...
            self.state = "AdjustRoll"

        if self.state is "AdjustRoll":
            ...
            fCmd.elevator = self.pitch_PID.control(fDat.pitch, [commanded_pitch, 0], fDat.time)
            fCmd.throttle = 0.7
            fCmd.aileron = self.roll_PID.control(fDat.roll, [0, 0], fDat.time) 
            if(self.duration > 5):
        ...
        if self.state is "AdjustAngle":
            ...
            if self.desired_pitch - fDat.pitch > 2:
                ...
                commanded_pitch = self.desired_pitch
            '''
            commanded_pitch = fDat.pitch + 5 if fDat.pitch < 80 else fDat.pitch - 5
            print('Commanded Pitch is: ' + str(commanded_pitch))
            if dt != 0:
                self.myflag = 1
            #fCmd.elevator = -1 * abs(self.pitch_PID.control(fDat.pitch, [commanded_pitch, 0], fDat.time))
            fCmd.elevator = -0.9
            if self.desired_speed - fDat.kias > 20:
                fCmd.throttle = 1.0
            elif fDat.kias - self.desired_speed < 20 and fDat.kias - self.desired_speed  > 0: 
                fCmd.throttle = 0.7
            else:
        ...
        print('Current State: ', self.state, '\n',
              'Current elevator', fCmd.elevator, '\n',
